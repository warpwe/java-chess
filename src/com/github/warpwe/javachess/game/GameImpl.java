/*
 * GameImpl - A class to store a complete chess game and provide access to all the game stages.
...
package com.github.warpwe.javachess.game;

import java.util.Iterator;

import com.github.warpwe.javachess.ply.IPly;
import com.github.warpwe.javachess.position.Position;
import com.github.warpwe.javachess.util.UnsynchronizedArrayStack;
...
public class GameImpl implements IGame {
  ...
  class GameStatus {
    ...
    /**
     * This bitmap indicates the pieces, that were moved from their initial position.
     */
    long movedPieces;
    ...
    private final void setMoved(Position position) {
      movedPieces |= (1L << position.getSquareIndex());
    }
    ...
    /**
     * Check if a piece on a given initial position has been moved.
     */
    public final boolean hasBeenMoved(Position position) {
      return ((movedPieces & (1L << position.getSquareIndex())) != 0L);
    }
  }
  ...
  /**
   * This stack holds the plies with the associated game status. The reason for using a stack are
   * the search operations for a new ply. You have to do and undo a lot of plies.
   */
  private UnsynchronizedArrayStack gameStatus;
   ...
   */
  public GameImpl() {
    gameStatus = new UnsynchronizedArrayStack();
  }
  ...
  private final GameStatus getLastGameStatus() {
    return gameStatus.empty() ? null : (GameStatus) gameStatus.peek();
  }
  ...
  public final void doPly(IPly ply) {
    ...
    gameStatus.push(getLastGameStatus() == null ? new GameStatus(ply) : new GameStatus(ply,
        getLastGameStatus()));
  }
  ...
  public final String toString() {
    ...
    for (Iterator<?> iter = gameStatus.iterator(); iter.hasNext();) {
