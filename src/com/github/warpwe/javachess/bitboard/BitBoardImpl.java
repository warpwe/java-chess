/*
 * BitBoardImpl - A class, that implements a chess board data structure as layered bitmaps.
...
package com.github.warpwe.javachess.bitboard;
...
public class BitBoardImpl implements IBitBoard {
  ...
  // Instance variables
  /**
   * Store the board in 4 64 bit layers. Layer 0 holds the color, layer 1-3 hold the piece code or
  ...
  public final IPiece getPiece(Position position) {
    ...
    int pieceType = (((int) (_boardLayer[1] >> bitpos) & 1)
        | (((int) (_boardLayer[2] >> bitpos) & 1) << 1) | (((int) (_boardLayer[3] >> bitpos) & 1) << 2));

    return (pieceType == 0) ? null : new PieceImpl(
        (byte) ((pieceType << 1) | ((_boardLayer[0] >> bitpos) & 1)));
  }
  ...
  public final void doPly(IPly ply) {
    ...
    if (ply instanceof ICastlingPly) {
      ...
      if (ply instanceof ITransformationPly) {
        ...
        setPiece(
            new PieceImpl(((ITransformationPly) ply).getTypeAfterTransformation(),
                movedPiece.getColor()), ply.getDestination());
      }
      else {
  ...
  public final long getPositionOfPieces(int pieceTypeColor) {
    return (((pieceTypeColor & 1) != 0 ? _boardLayer[0] : ~_boardLayer[0])
        ...
        & ((pieceTypeColor & 4) != 0 ? _boardLayer[2] : ~_boardLayer[2]) & ((pieceTypeColor & 8) != 0 ? _boardLayer[3]
        : ~_boardLayer[3]));
  }
   ...
   *
   * @param white
   *          true, if the white pieces are requested, false for the black pieces.
   */
  public final long getAllPiecesForColor(boolean white) {
    return ((_boardLayer[1] | _boardLayer[2] | _boardLayer[3]) & (white ? _boardLayer[0]
        : ~_boardLayer[0]));
  }
