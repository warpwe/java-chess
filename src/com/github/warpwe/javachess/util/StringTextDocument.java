/*
	StringTextDocument - The document as edit template for JTextField in the dialog
												to setup/edit the board position.
...
/**
 * Description:   This class is used to verify validate inputs in JTextFields
 *                such as in the setup/edit position dialog
 *                For further detailed information look at
 *                core Java 2 part 1 basics, chapter 9.3.2 p.496ff
 *                (Sun microsystems, Horstmann/Cornell)
 *                ISBN 3-8272-6016-7, ca. 50 EUR
 ...
 * Constructor
 *
 * @param characterCount The number of allowed character to enter in the JTextField
 */
 ...
 *
 * @param characterCount The number of allowed character to enter in the JTextField
 * @param checkForNumbers Flag whether the string to enter has to be parsed for
 *                        numbers >0 only
 */
 ...
 * @return <code>true</code> if the string is valid, based on the user's conditions
 ...
 * @param charToCheck The character to check
 * @return <code>true</code> when between a and h
 */
	private boolean isValidLine(char charToCheck)
	{
		if (( charToCheck >= 'a' && charToCheck <= 'h' )
					|| ( charToCheck >= 'A' && charToCheck <= 'H' ))
		{
			return true;
		}
		else{
 ...
 * @param bEnPassantSquare Flag to check for en passant (3rd row) or not
 *
